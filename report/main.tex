...
In our comparative evaluation of reinforcement learning architectures, we observe a distinct performance hierarchy between on-policy and off-policy methodologies. While Proximal Policy Optimization (PPO) serves as a stable baseline, it consistently exhibits lower sample efficiency due to its inability to reuse historical data. Conversely, TD3 and SAC (Off-Policy) models demonstrate high sample efficiency and stability by leveraging a replay buffer, which allows for the exhaustive re-evaluation of past transitions. This structural advantage allows off-policy agents to converge to higher reward thresholds with fewer environment interactions, proving that the capacity to "re-examine" experience is critical in both low-dimensional and high-dimensional continuous control.
