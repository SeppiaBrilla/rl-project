...
\section{Experiments}
Figure \ref{fig:results} illustrates the training progression of PPO, SAC, and TD3 across three distinct control environments: CarRacing, CartPole, and Acrobot. All algorithms use the same neural network architecture, with 256 hidden nodes in both the first and second layer, and ReLU activation functions in the intermediate layers. To account for the image-based input in the CarRacing environment, a convolutional neural network (hyperparameters summarized in Table \ref{tab:cnn_hyperparams}) is used to extract features from the input and pass it to the base network which is shared with the other two environments. The same random seeds are used to train all models. For each experiment, we perform 5 independent runs with different random seeds. To ensure a reliable estimate of the policy's performance during training, each evaluation point within a single run is calculated as the average return over 5 separate test executions. Consequently, the results presented in this report represent an \textbf{average of an average} across these runs. Detailed hyperparameters for each algorithm across the three environments are summarized in Table \ref{tab:ppo_hyperparams}, Table \ref{tab:sac_hyperparams}, and Table \ref{tab:td3_hyperparams}.
...
\textbf{Acrobot Environment.} The Acrobot environment is notoriously challenging, and initial baseline implementations of the algorithms failed to solve the task. This difficulty can be attributed to several compounding factors inherent to the environment. First, it is severely underactuated: the agent controls only the elbow joint torque while needing to coordinate two links to swing up. This requires discovering a precise "energy pumping" strategy through complex multi-step action sequences. Second, the chaotic nature of double-pendulum dynamics means that small variations in actions lead to vastly different and unpredictable trajectories, making both value function approximation and policy gradient estimation extremely noisy and high-variance. Third, the learning signal is exceptionally sparse: random exploration almost never produces upward swings, providing minimal gradients for learning. Finally, successful swing-ups require long sequences of coordinated actions, creating a severe credit assignment problem where the impact of early actions is discounted by factor $\gamma^t$, making it difficult for temporal difference methods to propagate value information backward effectively.

To overcome these inherent difficulties and enable the agent to successfully learn the swing-up behavior, we introduced a series of crucial algorithmic enhancements that directly address the environment's complexities:
\begin{itemize}
  \item \textbf{Action Repeats:} To mitigate the credit assignment problem and facilitate the "energy pumping" strategy, we implemented momentum-building action repeats. This effectively shortens the temporal horizon and allows the agent to commit to sustained torque applications, stabilizing the chaotic dynamics.
  \item \textbf{Upright-Start Initialization:} We modified the environment to initialize episodes near the target upright position. This curriculum-based approach allows the agent to first master the stabilization and balancing task before attempting the more difficult full swing-up phase, significantly easing the initial exploration burden. This is also a crucial reason why the reward is so high. And all algorithms perform similarly.
  \item \textbf{Reward Shaping:} We applied and carefully tuned a shaped reward function (see Equation \ref{eq:reward_shaping}) to provide a dense gradient towards the upright position, while resolving conflicts between shaped and sparse reward signals.
  \item \textbf{Gradient Clipping:} To handle the high variance in policy gradient estimation and value function approximation caused by the environment's chaotic nature, we applied gradient clipping to ensure training stability.
\end{itemize}
...
With these modifications, the agent is now capable of successfully learning the environment, as reflected in the learning curves. It is worth noting that the magnitude of the final cumulative reward is remarkably high. This is a direct consequence of the continuous reward shaping mechanism: terms like the tip height provide positive scalar feedback at every time step. When the agent learns to swing up and balance the pole in the upright position, these positive values accumulate over the hundreds of steps in an episode, resulting in a large total episodic return.

\textbf{Caveat on Training Constraints:} Despite these successes, we must note a limitation regarding the total number of training steps. Due to strict computational resource constraints, the training process was limited in duration. Consequently, while the agent successfully discovers the swing-up and balancing behavior, the learning curves may not reflect full convergence to a perfectly optimal, asymptotic policy. We expect that with a larger computational budget and extended training, the agent would further refine its control strategy and exhibit even greater stability. This limited training, paired with the high reward signal from the shaped reward function, is also the reason why it is not possible to see any meaningful performance difference between the three algorithms.

\section{Conclusion}

\paragraph{AI Usage Disclosure}
Gemini 3.0 was used to generate the figures and tables presented in this report as well as assisting in writing the code for the experiments. All generated code, text and figures was reviewed, edited and corrected by the author who takes full responsibility for the content of this report.
