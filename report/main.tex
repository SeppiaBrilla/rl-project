...
The figure below illustrates the training progression of PPO, SAC, and TD3 across three distinct control environments: CarRacing-v2 (High-dimensional vision), CartPole-v1 (Low-dimensional stability), and AcroBot-v1 (Swing-up dynamics). All the algorithms use the same neural network, with 256 hidden nodes in
...
In our comparative evaluation of reinforcement learning architectures across the CartPole, CarRacing, and Acrobot environments, we observe a nuanced performance hierarchy between on-policy and off-policy methodologies. Typically, TD3 and SAC (Off-Policy) models demonstrate high sample efficiency and stability by leveraging a replay buffer, which allows for the exhaustive re-evaluation of past transitions. This structural advantage often allows off-policy agents to converge to higher reward thresholds with fewer environment interactions, proving that the capacity to "re-examine" experience is critical in both low-dimensional and high-dimensional continuous control. However, our results indicate that this advantage is environment-dependent; while off-policy methods dominate in tasks like CartPole and CarRacing, the on-policy Proximal Policy Optimization (PPO) serves as a superior stabilizer in chaotic, momentum-based tasks like Acrobot. This suggests that while data reuse drives peak performance, the "freshness" of on-policy data remains vital for maintaining stability in highly volatile state spaces.

In the CarRacing environment introduces the complexities of high-dimensional pixel inputs and non-linear physics, such as tire friction and centrifugal force during drifts. In this domain, the off-policy advantage becomes even more pronounced. The computational cost of rendering frames makes the sample efficiency of SAC and TD3 indispensable. TD3 and SAC (Off-Policy) models demonstrate high sample efficiency and stability by iteratively refining their strategies based on a repository of varied racing lines and crash scenarios. TD3 excels by minimizing Q-value overestimation, which prevents the agent from committing to aggressive, high-speed trajectories that appear optimal in the short term but lead to catastrophic failure (local optima). Meanwhile, SAC’s entropy-maximization allows it to maintain superior exploration, providing the agent with the necessary "practice" in unstable states, such as random drifts, to develop robust recovery behaviors. This comprehensive exploration of the state space often results in the highest final scores in environments with local optima, where a more rigid, on-policy agent like PPO would likely fail to generalize across the diverse topography of the track.

The CartPole environment, the primary challenge lies in the precision of the force applied to maintain a precarious equilibrium. As illustrated in the performance metrics, TD3 excels by minimizing Q-value overestimation through its twin-critic architecture. In the continuous action space of CartPole, standard actor-critic methods often develop an optimistic bias toward high-magnitude actions; TD3’s clipped double-Q learning suppresses this bias, resulting in a more disciplined and stable balancing act. Simultaneously, SAC’s entropy-maximization allows it to maintain superior exploration, ensuring that the agent does not settle for a "good enough" policy. By rewarding the agent for both success and policy diversity, SAC discovers a wider range of recovery maneuvers, which often results in the highest final scores in environments with local optima, significantly outpacing the performance of the PPO baseline.

Considering the Acrobot environment, which is sensitive to initial conditions and the timing of torque application. The learning curve reveals that PPO maintains the highest mean evaluation reward, albeit with a wide confidence interval that suggests the environment's inherent difficulty. TD3 excels by minimizing Q-value overestimation, but in Acrobot, this precision can sometimes lead to overly conservative movements that fail to generate the momentum required for the swing-up. Similarly, while SAC’s entropy-maximization allows it to maintain superior exploration, the resulting high-variance policy may struggle to converge on the hyper-specific sequence of actions needed to reach the goal height consistently.

The volatility seen in the SAC and TD3 curves, characterized by frequent performance dips, indicates that the replay buffer may be capturing "stale" or "misleading" transitions in this fast-moving environment. Because the goal is reached through a cumulative build-up of energy, off-policy critics may struggle to accurately credit rewards to specific actions taken many steps prior. In this context, PPO’s reliance on recent, on-policy data ensures that the gradients are always derived from the most current state of the agent's capabilities, preventing the catastrophic policy collapses often seen in the off-policy counterparts. This results in PPO achieving a more robust average performance, even if SAC’s exploration often results in the highest final scores in environments with local optima where a clear "path" to the goal is more easily mapped into a value function.
